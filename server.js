...
// ⚠️ сервисный ключ ТОЛЬКО на сервере (env), в HTML его быть не должно
const sb = createClient(SUPABASE_URL, SERVICE_KEY);

// (необязательно, но полезно) очень простой доступ к bulk-редактированию:
const ADMIN_TOKEN = process.env.ADMIN_TOKEN; // придумай длинный токен
...
app.get("/api/leaders/week", async (req, res) => {
  ...
  const { data, error } = await sb
    ...
    .select("week_start, week_end, nick_1, dept_1, ash_1, nick_2, dept_2, ash_2, nick_3, dept_3, ash_3, total_ash")
...
app.get("/api/leaders/by-depts", async (req, res) => {
  const depts = String(req.query.depts || "").split(",").map(s => s.trim()).filter(Boolean);
  const { data, error } = await sb
    .from("leaderinfo")
